...
const uploadPane = new Pane();

const uploadButton = uploadPane.addButton({ title: 'upload', label: 'JSON file'});

const jsonUploader = document.getElementById('fileInput');

uploadButton.on('click', () => {
    jsonUploader.click();
    jsonUploader.addEventListener("change", async (e) => {
        let file = e.target.files[0];
        console.log(typeof file);
        let reader = new FileReader();

        // This is code that runs after reader.readAsText() finishes
        reader.onload = function ( jsonText ) {

            var loader = new THREE.ObjectLoader();

            loader.parse( jsonText, function( json ){
...
const planeVisibilityCheckboxParams = {
    'show plane': false, //at first, box is unchecked so value is "false"
};
...
planeVisibilityCheckbox.on('change', () => { //on change, change the hidden and visibility values set
